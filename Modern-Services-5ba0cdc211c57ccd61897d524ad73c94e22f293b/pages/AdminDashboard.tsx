...
import { LogOut, CheckCircle, XCircle, Clock, User, MapPin, MessageSquare } from 'lucide-react';
  ...
  const loadTestimonials = async () => {
    try {
      ...
      if (error.code === 'failed-precondition') {
        setError('Firestore index missing. Run: firebase deploy --only firestore:indexes');
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-300 text-sm mt-1">Manage Testimonials</p>
            </div>
            <Button
                  ...
                  {error && (
                    <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded text-red-800 text-sm">
                      ...
                      <span className="text-xs mt-1 block">
                        Check browser console (F12) for details. See DEBUG_ADMIN_PANEL.md for troubleshooting.
                      </span>
                    </div>
                  )}
                </div>
